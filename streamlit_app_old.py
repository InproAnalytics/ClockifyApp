...
# ====== Navigation ======
if isinstance(st.session_state.get("pdf_bytes"), bytes):
    st.markdown("---")
    st.subheader("Bericht neu erstellen")
    
    # Кнопки навигации
    col1, col2, col3 = st.columns(3)
    
    # 1. Neuer Zeitraum (полный сброс)
    with col1:
        if st.button("🔄 Neuer Zeitraum"):
            # Очищаем все состояния, außer Authentifizierung
            for key in list(st.session_state.keys()):
                if key not in ["authenticated", "username", "api_key", "workspace_id", "base_url", "lang"]:
                    del st.session_state[key]
            st.rerun()
    
    # 2. Neuer Kunde (Zeitraum beibehalten)
    with col2:
        if st.button("👥 Neuer Kunde"):
            # Save only the period data
            period_data = {
                "start_date": st.session_state.get("start_date"),
                "end_date": st.session_state.get("end_date"),
                "date_range_type": st.session_state.get("date_range_type"),
                "date_input_key": st.session_state.get("date_input_key"),
                "data_loaded": True,
                "df_date": st.session_state.get("df_date"),
                "confirmed_period": True  # This keeps us from going back to period confirmation
            }
            
            # Clear client and project related states
            keys_to_clear = [
                "client_selectbox", "client_selected", "client_name",
                "selected_projects", "multiselect_projects",
                "selected_all_projects", "final_confirmed",
                "editor_table", "editable_table", "pdf_bytes"
            ]
    ...
    # 3. Change Projects (keep client and period)
    with col3:
        if st.button("📋 Projekte ändern"):
            if st.session_state.get("client_selected"):
                
                # ----- сохранить период и клиента -----
                client_period_state = {
                    "client_selected": st.session_state["client_selected"],
                ...
                for key in ["selected_projects", "selected_all_projects",
                            "multiselect_projects", "editor_table", "editable_table"]:
                    st.session_state.pop(key, None)

                # ----- обновить -----
                st.session_state.update(client_period_state)

                st.rerun()
            
            st.markdown("---")
    
    # Download- und Beenden-Buttons
